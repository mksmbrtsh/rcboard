...
namespace UPnP
{
    public class NAT
    {
        static TimeSpan _timeout = new TimeSpan(0, 0, 0, 2);
            ...
            do
            {
                ...
                do
                {
                    ...
                    if (resp.Contains("upnp:rootdevice") 
                        ...
                        || resp.Contains("urn:schemas-upnp-org:service:WANIPConnection")
                        || resp.Contains("urn:schemas-upnp-org:service:WANPPPConnection")
                    )
                    {
        ...
        private static string GetServiceUrl(string resp)
        {
            ...
            try
            {
                ...
                if (!typen.Value.Contains("InternetGatewayDevice") && !typen.Value.Contains("upnp:rootdevice") && !typen.Value.Contains("service:WANIPConnection") && !typen.Value.Contains("service:WANPPPConnection"))
                    return null;
        ...
        public static void ForwardPort(String host, int port, ProtocolType protocol, string description)
        {
            ...
            XmlDocument xdoc = SOAPRequest(_serviceUrl, "<u:AddPortMapping xmlns:u=\"urn:schemas-upnp-org:service:WANIPConnection:1\">" +
                ...
                "<NewInternalPort>" + port.ToString() + "</NewInternalPort><NewInternalClient>" + host + "</NewInternalClient><NewEnabled>1</NewEnabled><NewPortMappingDescription>" + description +
            "</NewPortMappingDescription><NewLeaseDuration>0</NewLeaseDuration></u:AddPortMapping>", "AddPortMapping");
        }

        public static void DeleteForwardingRule(String host, int port, ProtocolType protocol)
        {
            if (string.IsNullOrEmpty(_serviceUrl))
                throw new Exception("No UPnP service available or Discover() has not been called");
